...
#导出数据库中基于板块的技术因子CSV
def export_data_by_sector_and_date(db_config, sector_name, start_date, end_date, indicator_names, output_csv_path):
    """
    ...
    try:
        ...
        """
        stock_codes = pd.read_sql(stock_query, engine)['stock_code'].tolist()
        ...
        AND t.stock_code IN ({", ".join([f"'{code}'" for code in stock_codes])})
        """
        ...
        AND h.stock_code IN ({", ".join([f"'{code}'" for code in stock_codes])})
        """
...
if __name__ == "__main__":
    ...
    end_date = '2024-12-01'
    indicator_names = ['dividend_per_share']
    output_csv_path = 'bank_per_share.csv'
    export_data_by_sector_and_date(db_config, sector_name, start_date, end_date, indicator_names, output_csv_path)